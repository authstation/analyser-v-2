...
export function sanitizeUser(user: any) {
  return {
    ...
    planName: user.planName ?? user.plan?.name ?? null,
    planPrice: user.planPrice ?? user.plan?.price ?? null,
    planDurationDays: user.planDurationDays ?? user.plan?.durationDays ?? null,
    planStartDate: user.planStartDate ? String(user.planStartDate) : null,
    trxId: user.trxId || null,
    hasChangedCircle: user.hasChangedCircle ?? false,
...
export async function issueTokens(c: any, user: any, options?: { remember?: boolean }) {
  ...
  // user.role is a plain string enum ("admin"/"user"), not an object
